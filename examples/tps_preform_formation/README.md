# TPS preform formation

Start with the **[Python notebook](../../crates/tangle_python/python/examples/native/tps_preform_formation.ipynb)**
or [editable script](../../crates/tangle_python/python/examples/native/tps_preform_formation.py).
...
[![A staged TPS-preform manufacturing recipe](../../docs/media/formation-recipe.png)](../../docs/media/formation-recipe.mp4)
...
```console
cargo run --release -p tps_preform_formation
```

Record the staged process as an OVITO-readable trajectory:

```console
cargo run --release -p tps_preform_formation -- --debug-ovito
```
...
```console
python examples/tps_preform_formation/render_movie.py
```

The movie is written to `output/tps_preform_formation.mp4`.
...
This is intentionally a hypothetical material, not a calibrated FiberForm or MERINO
