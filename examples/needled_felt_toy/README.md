...
Start with the **[Python notebook](../../crates/tangle_python/python/examples/native/needled_felt_toy.ipynb)**
or [editable script](../../crates/tangle_python/python/examples/native/needled_felt_toy.py).
...
```console
cargo run --release -p needled_felt_toy
```

Record an OVITO-readable trajectory:

```console
cargo run --release -p needled_felt_toy -- --debug-ovito
```
