//! Formation recipes and population specifications for the twenty-ply felt.
...
pub(super) fn dem_polish_recipe() -> Vec<FormationOperation> {
    vec![
        flexible_contact_cleanup("polish/1-contact-coarse", 0.05e-6, FINAL_BEND_RATIO, 75_000),
        flexible_contact_cleanup(
            "polish/2-contact-final",
...
pub(super) fn cleanup_recipe() -> Vec<FormationOperation> {
    vec![
        ...
        FormationOperation::SetMaterialBendRadius {
            material_name: "fine_7um".to_string(),
        ...
        curvature_cleanup("cleanup/1-curvature-coarse", 2.0, 20_000),
        flexible_contact_cleanup("cleanup/2-contact-coarse", 2.0e-6, 1.25, 30_000),
        curvature_cleanup("cleanup/3-curvature-near", 1.25, 40_000),
        flexible_contact_cleanup("cleanup/4-contact-near", 0.75e-6, 1.05, 60_000),
        curvature_cleanup("cleanup/5-curvature-final", FINAL_BEND_RATIO, 100_000),
        flexible_contact_cleanup(
...
pub(super) fn mechanics_recovery() -> FormationOperation {
    FormationOperation::RelaxWithOverrides {
        policy: SolvePolicy {
            name: "settle/2-mechanics-recovery".to_string(),
...
pub(super) fn contact_first_cleanup() -> FormationOperation {
    FormationOperation::RelaxWithOverrides {
        policy: SolvePolicy {
            name: "settle/1-contact-first".to_string(),
            solver_targets: RelaxationTargets {
                penetration: 1.0e-6,
                curvature_ratio: 5.0,
            },
...
pub(super) fn small_fibers() -> FiberPopulationSpec {
    FiberPopulationSpec {
        ...
        ..common_population(SMALL_FIBERS, "fine_7um", 20_260_921)
    }
}
...
pub(super) fn large_fibers() -> FiberPopulationSpec {
    FiberPopulationSpec {
        ...
        ..common_population(LARGE_FIBERS, "coarse_19um", 20_260_922)
    }
}
...
pub(super) fn print_population_design() {
    ...
    println!("twenty-ply felt population design:");
