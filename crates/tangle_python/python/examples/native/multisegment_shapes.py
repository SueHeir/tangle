...
OUTPUT = Path(__file__).parent / "output" / "multisegment_shapes"
MINIMUM_BEND_RADIUS = 0.12
...
def build(case_name: str) -> tuple[tangle.Recipe, tangle.RelaxationSettings]:
    ...
    settings.stretch_stiffness = 0.35
    settings.bend_stiffness = 0.03
    settings.curvature_limit_stiffness = 1.0
    settings.curvature_ratio_tolerance = 1.0e-4
    settings.max_step = 0.008
    settings.max_iterations = 5_000
    settings.iterations_per_batch = 128
