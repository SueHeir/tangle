...
impl<R: Runtime> DeviceFiberWorld<R> {
    ...
    pub fn upload(
        ...
        // Segments longer than `proxy_length` are binned as several equal
        // proxy pieces, so the grid follows the short segments. In a uniform
        // layout every segment is a leaf and the proxy length is the longest
        // segment, which bins each segment whole as before. An adaptive layout
        // bins unrefined parents as pieces no longer than its longest leaf, or
        // twice the interaction margin, which balances slots per segment
        // against candidates per slot.
        let maximum_leaf_length = (0..packed.segment_count())
            .filter(|segment| packed.segment_children[2 * segment] == u32::MAX)
            .map(|segment| packed.segment_rest_lengths[segment].max(initial_length(segment)))
            .fold(0.0_f32, f32::max);
        let proxy_length = maximum_length.min(maximum_leaf_length.max(2.0 * interaction_margin));
        let segment_proxies: Vec<u32> = (0..packed.segment_count())
            .map(|segment| {
                let length = packed.segment_rest_lengths[segment].max(initial_length(segment));
                ((length / proxy_length).ceil() as u32).max(1)
            })
            .collect();
    ...
    pub fn unpack_positions(
                ...
                .any(|step| *step > 0)
        {
            ...
            self.packed
                .unpack_active_positions(positions, &active, assembly)
        } else {
            self.packed.unpack_positions(positions, assembly)
        }
    }
